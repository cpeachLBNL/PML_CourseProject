...
head(training)
str(training)
#Check if any NA left in dataset
print (paste("All NA removed:  ", any(is.na(training))==FALSE))
#[1] "All NA removed:   TRUE"

#Use Random Subsampling to create 10 sets of training and cross-validation data sets. 
#On each loop, train using Random Forest model on train dataset
#  and calculate accuracy on Cross Validation (xvalid) dataset
#Keep a list of all Random Forest objects and accuracy
lstRF <- list()
nRuns <- 10
sumAcc <- 0
for (n in 1:nRuns){
